"use client"

import { useEffect } from "react"
import Image from "next/image"
import { Button } from "@/components/ui/button"
import { Card, CardContent } from "@/components/ui/card"
import { URLS } from "@/constants/urls"
import { useAudio } from "@/contexts/AudioContext"
import Link from "next/link"

export default function SadStoryPage() {
  const { isPlaying, toggleAudio } = useAudio()

  useEffect(() => {
    if (!isPlaying) {
      toggleAudio()
    }
  }, [isPlaying, toggleAudio])

  return (
    <div className="min-h-[100dvh] bg-[#8B5CF6] flex items-center justify-center p-4">
      <Card className="w-full max-w-md bg-transparent border-none shadow-none">
        <CardContent className="flex flex-col items-center space-y-6 p-4">
          <div className="text-center space-y-2">
            <h1 className="text-3xl font-bold text-white">thiago ta triste :(</h1>
            <p className="text-xl text-white">ele quer saber se vc quer ouvir a triste historia dele</p>
          </div>

          <div className="w-full h-auto aspect-square relative rounded-lg overflow-hidden max-w-[280px] mx-auto">
            <Image
              src={URLS.IMAGES.IMAGEM_AJUSTE_2 || "/placeholder.svg"}
              alt="Sad puppy"
              fill
              className="object-cover"
              priority
            />
          </div>

          <p className="text-lg text-white text-center">aproveita e ja liga o som do celular</p>

          <div className="w-full max-w-[280px]">
            <Link href="/5">
              <Button
                variant="default"
                className="w-full bg-black hover:bg-gray-800 text-white rounded-full py-6 text-lg"
              >
                Quero
              </Button>
            </Link>
          </div>

          <p className="text-sm text-white/60 text-center">sim so tem essa opcao</p>
        </CardContent>
      </Card>
    </div>
  )
}

